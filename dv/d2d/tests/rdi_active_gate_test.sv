module ucie_d2d_test (
  ...
  task automatic expect_no_init_activity(input string phase);
    if (rdi.lpCfgVld === 1'b1) begin
      $fatal(1, "Adapter sent initialization sideband before RDI Active during %s", phase);
    end
  ...
  initial begin
    ...
    if (!sb_is_advcap_adapter(msg)) begin
      $fatal(1, "Expected ADV_CAP after RDI Active, got 0x%032h", msg);
    end

    if (fdi.plStateSts === RDI_STATE_ACTIVE) begin
      $fatal(1, "FDI reached Active immediately after first ADV_CAP");
    end

    $display("D2D rdi_active_gate completed");
    $finish;
  end
endmodule
