...
class SidebandLinkSerdesTest extends AnyFunSpec with ChiselSim {
  ...
  describe("Sideband Link Serializer and Deserializer Instantiation Test") {
    ...
    it("Instantiated Deserializer") {
      ...
      simulate(new SidebandLinkDeserializer(sbLink_w, msg_w, timeoutCycles)) {
        c =>
          c.clock.step()
          println("[TEST] Success")
      }
    ...
    it(s"Serialized ${numPackets} 64-bit packet(s)") {
      ...
      { c =>
        ...
        for (i <- 0 until numPackets) {
          ...
          val opcodes64Bits = Seq(
            SBMsgOpcode.MessageWithoutData,
  ...
  class DeserializerTestHarness(sbLink_w: Int, msg_w: Int, timeoutCycles: Int)
      extends Module {
    ...
    val dut = Module(
      new SidebandLinkDeserializer(sbLink_w, msg_w, timeoutCycles)
    )
    ...
    it(s"Stopped serializing a packet when reset was triggered") {
      ...
      { c =>
        ...
        for (i <- 0 until numPackets) {
          ...
          println(
            "[TEST] Captured Data (Binary): " + toBits(capturedData, bitWidth)
          )
