...
class SBInitSMTest extends AnyFunSpec with ChiselSim {

  describe("SBInitSM Instantiation Test") {
    it("SBInitSM Instantiated") {
      simulate(new SBInitSM(new SidebandParams(), 8000000))
      { c =>
        c.clock.step()
  ...
  class SBInitSMTestHarnessLoopback(sbParams: SidebandParams) extends Module {
    ...
    class TXArbiter(sbParams: SidebandParams) extends Module {
      val io = IO(new Bundle {
        ...
        val reqTx =
          Flipped(Decoupled(new SidebandLanes(sbParams.sbNodeMsgWidth)))
        val respTx =
          Flipped(Decoupled(new SidebandLanes(sbParams.sbNodeMsgWidth)))
      })
    ...
    val rxQueue = Module(
      new Queue(Bits(sbParams.sbNodeMsgWidth.W), sbParams.sbLinkAsyncQueueDepth)
    )
  ...
  describe("SBInitSM Loopback Test") {
    it("SBInitSM loopback configuration works") {
      simulate(new SBInitSMTestHarnessLoopback(sbParams))
      { c =>
