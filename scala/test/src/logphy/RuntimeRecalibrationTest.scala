...
class RuntimeRecalibrationTest extends AnyFunSpec with ChiselSim {
  ...
  describe("Runtime recalibration behavior in Active") {
    it("keeps adapter-facing RDI in Active without asserting stall during recenter activity") {
      simulate(new RdiControllerLoopbackHarness(new SidebandParams())) { dut =>
        ...
        var i = 0
        while (i < 20) {
          dut.io.plStateSts.expect(RDIState.active)
          dut.io.plInbandPres.expect(true.B)
          dut.io.plStallReq.expect(false.B)
          stepWithClkAck(dut)
          i += 1
        }

        // Recalibration/recenter window with sideband activity present.
        dut.io.plPhyInRecenter.poke(true.B)
