...
class SidebandLinkSerdesTest extends AnyFunSpec with ChiselSim with VerilatorCoverage {
  ...
  describe("Sideband Link Serializer and Deserializer Instantiation Test") {
    ...
    it("Instantiated Deserializer") {
      ...
      simulate(new SidebandLinkDeserializer(sbLinkW, msgW, timeoutCycles)) { c =>
        c.clock.step()
        printDebug("[TEST] Success")
      }
    }
  ...
  def dataForOpcode(opcode: SBMsgOpcode.Type, bitWidth: Int): BigInt = {
    ...
    val pattern = (BigInt(1) << (bitWidth - 1)) | (BigInt("5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a5a", 16) & (widthMask >> 1))
    ...
    it(s"Serialized ${numPackets} RAW packet(s) (64 bits per packet)") {
      simulate(new SidebandLinkSerializer(sbLinkW, msgW)) { c =>
        ...
        for(i <- 0 until numPackets) {
          ...
          if(i != (numPackets - 1)) {
            printDebug(s"[TEST] Waiting ${waitAmt} cycles before sending next message")
          }
          if (waitAmt != 0) {
  ...
  describe("Serialize 64-bit sideband packet(s)") {
    ...
    it(s"Serialized ${numPackets} 64-bit packet(s)") {
      simulate(new SidebandLinkSerializer(sbLinkW, msgW)) { c =>
        ...
        printDebug(s"[TEST] Starting: Serializing ${numPackets} 64-bit packet(s)")
        // initialize
        ...
        for(i <- 0 until numPackets) {
          ...
          printDebug("[TEST] Selected opcode bits:  " + toBits(randomOpcode.litValue, bitWidth))
          val randData = (BigInt(bitWidth - 5, rand)) << 5
          printDebug("[TEST] Generated random bits: " + toBits(randData, bitWidth))
  ...
  describe("Serialize 128-bit sideband packet(s)") {
    ...
    it(s"Serialized ${numPackets} 128-bit packet(s)") {
      simulate(new SidebandLinkSerializer(sbLinkW, msgW)) { c =>
        ...
        printDebug(s"[TEST] Starting: Serializing ${numPackets} 128-bit packet(s)")
        // initialize
        c.io.ctrl.txMode.poke(SBRxTxMode.PACKET)
        c.io.in.valid.poke(false.B)
        c.clock.step(10)
  ...
  describe("Serialize any sideband packet(s)") {
    ...
    it(s"Serialized ${numPackets} sideband packet(s)") {
      simulate(new SidebandLinkSerializer(sbLinkW, msgW)) { c =>
        ...
        printDebug(s"[TEST] Starting: Serializing ${numPackets} sideband packet(s)")
        // initialize
        c.io.ctrl.txMode.poke(SBRxTxMode.PACKET)
        c.io.in.valid.poke(false.B)
        c.clock.step(10)
    ...
    it(s"Stopped serializing a packet when reset was triggered") {
      simulate(new SidebandLinkSerializer(sbLinkW, msgW)) { c =>
        ...
        for(i <- 0 until numPackets) {
          ...
          printDebug(s"[TEST] Waiting ${resetWaitAmt} cycles before triggering reset")
          var bitstring = ""
  ...
  def expectDeserializerPacket(c: DeserializerTestHarness, data: BigInt, bitWidth: Int): Unit = {
    var guard = 0
  ...
  def resetDeserializer(c: DeserializerTestHarness, rxMode: SBRxTxMode.Type): Unit = {
  ...
  def driveDataEdges(c: DeserializerTestHarness, data: BigInt, start: Int, count: Int): Unit = {
  ...
  def expectNoDeserializerOutput(c: DeserializerTestHarness, cycles: Int): Unit = {
  ...
  class DeserializerTestHarness(sbLinkW: Int, msgW: Int, timeoutCycles: Int) extends Module {
    ...
    it("Held valid data stable until consumed") {
      ...
      { c =>
        def checkBackpressure(opcode: SBMsgOpcode.Type): Unit = {
          ...
          assert(heldBits == data, s"[TEST] Error in backpressured output for opcode $opcode")
        }
    ...
    it(s"Stopped serializing a packet when reset was triggered") {
      ...
      { c =>
        ...
        for(i <- 0 until numPackets) {
          ...
          printDebug("[TEST] Number of cycles before triggering reset: " + resetWaitAmt)
          ...
          */
          ...
          for(i <- 0 until 50) {
            c.io.out.msg.valid.expect(false.B, "Should not output a partial packet")
            c.clock.step()
          }
          printDebug("[TEST] Success")
          c.reset.poke(false.B)
    ...
    it(s"Triggered a timeout when serializer stopped sending a packet") {
      ...
      { c =>
        ...
        for(i <- 0 until numPackets) {
          ...
          printDebug("[TEST] Number of cycles before killing serializer: " + timeoutWaitAmt)
  ...
  def expectLoopbackPacket(c: LinkSerdesTestHarness, data: BigInt, bitWidth: Int): Unit = {
    var guard = 0
    ...
    val fullStr =
      c.io.deserializerIO.out.msg.bits.peek().litValue.toString(2).reverse.padTo(bitWidth, '0').reverse
    val capturedData = if(bitWidth == 64) fullStr.takeRight(64) else fullStr
  ...
  describe("Sanity test using async queues and a loopback configuration") {
    ...
    it("Preserved the 32-bit wait with a queued back-to-back packet") {
      simulate((new LinkSerdesTestHarness))
      { c =>
        def checkPair(firstOpcode: SBMsgOpcode.Type, secondOpcode: SBMsgOpcode.Type): Unit = {
          ...
          val cyclesToFinalWait = firstWidth + (((firstWidth / 64) - 1) * 32) + 1
          ...
          for(_ <- 0 until 32) {
            ...
            acceptedSecond ||= c.io.serializerIO.in.msg.ready.peek().litToBoolean
            c.clock.step()
            if(acceptedSecond) {
              c.io.serializerIO.in.msg.valid.poke(false.B)
            }
          ...
          assert(acceptedSecond, "Serializer did not accept the queued back-to-back packet")
          expectLoopbackPacket(c, firstData, firstWidth)
          expectLoopbackPacket(c, secondData, secondWidth)
        }

        checkPair(SBMsgOpcode.CompletionWithoutData, SBMsgOpcode.MemoryWrite_64b)
        checkPair(SBMsgOpcode.MemoryWrite_64b, SBMsgOpcode.CompletionWithoutData)
      }
    }
      ...
      { c =>
        ...
        for(opcode <- SBMsgOpcode.all) {
          ...
          while(!c.io.deserializerIO.out.msg.valid.peek().litToBoolean) {
            c.clock.step()
            guard += 1
            assert(guard < 5000, s"Loopback: opcode $opcode never produced output")
          }
          ...
          assert(capturedData == toBits(data, bitWidth), s"[TEST] Error in loopback for opcode $opcode")
          c.clock.step(50)
        }
      }
    }
