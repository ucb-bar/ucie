...
class MainbandLaneController(afeParams: AfeParams, rdiParams: RdiParams)
    extends Module {
  ...
  require(
    nLanes == 8 || nLanes == 16,
    "only 8- and 16-lane configurations are supported"
  )
  ...
  def calcBeatsForActive(activeLanes: Int): UInt = {
    require(
      nBytes % (activeLanes * bytesPerLane) == 0,
  ...
  val txStart =
    !txBusy && io.rdi.tx.lpValid && io.rdi.tx.lpIrdy && io.mbLanes.tx.ready
  ...
  val txDataBytes = VecInit(
    Seq.tabulate(nBytes)(i => txEffectiveData(i * 8 + 7, i * 8))
  )
