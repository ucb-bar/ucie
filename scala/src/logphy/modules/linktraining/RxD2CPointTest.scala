...
class RxD2CPointTestRequester(afeParams: AfeParams, sbParams: SidebandParams) extends Module {
  val io = IO(new Bundle {
    ...
    val rxInitPtTestLocalResults = Output(Valid(Vec(afeParams.mbLanes, UInt(1.W))))  
  })
  ...
  val rxInitPtTestLocalResults = WireInit(VecInit(Seq.fill(afeParams.mbLanes)(false.B)))
  switch(comparisonModeReg) {
    is(1.U) { // Aggregate comparison
      rxInitPtTestLocalResults(0) := io.patternReaderIo.resp.bits.aggregateStatus
    }
    is(0.U) { // Per-lane comparison
      for(i <- 0 until afeParams.mbLanes) {
        rxInitPtTestLocalResults(i) := io.patternReaderIo.resp.bits.perLaneStatusBits(i)
      }
    }
  }
