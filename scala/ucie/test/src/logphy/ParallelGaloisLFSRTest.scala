      ...
      simulate(new ParallelGaloisLFSR(seed, lfsrWidth, dataWidth, polynomial)) {
        c =>
          ...
          for (i <- 0 until numIncrements) {
            ...
            val dutOutputBitStr = c.io.lfsrOutput
              .peek()
              .litValue
              .toString(2)
              .reverse
            ...
            val dutStateHexStr =
              c.io.state.peek().litValue.toString(16).toUpperCase
            println(
              s"[TEST] Reference state after incrementing: 0x${refLfsr.getStateHex()}"
            )
            println(
              s"[TEST] DUT state after incrementing:       0x${dutStateHexStr}"
            )

            assert(
              refLfsr.getStateHex() == dutStateHexStr,
        ...
        c =>
          ...
          for (i <- 0 until numIncrements) {
            ...
            if (i == (whenToReset - 1)) {
              ...
              println(
                s"[TEST] DUT state after reset:       0x${dutStateHexStr}"
              )
