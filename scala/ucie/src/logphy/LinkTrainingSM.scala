...
object TimeoutConstants {
  def timeoutMap(
      mbSerializerRatio: Int,
      timeoutMs: Double = 0.008
  ): Map[SpeedMode.Type, BigInt] = {
...
class SidebandCtrlIO extends Bundle {
  ...
  val sbSerDesRst = Output(
    Bool()
  ) // TODO: Planning to toggle this high during the 4ms wait coming into reset
...
class LinkTrainingSM(
  ...
  val io = IO(new Bundle {
    ...
    val retryTrainingAmt =
      Input(UInt(retryAmtW.W)) // comes from ucie dvsec (controller in logphy)

    // NOTE: swTrigger from DVSEC regs, rdiTrigger from adapter
    //       get both signals from logPHY controller
    val swTriggerTraining =
      Input(Bool()) // from dvsec regs (controller in logphy)
    val rdiTriggerTraining =
      Input(Bool()) // from adapter (controller in logphy)
  ...
  val timeoutMapScala =
    TimeoutConstants.timeoutMap(mbSerializerRatio, timeoutMs)
  val timeoutWidth = log2Ceil(timeoutMapScala.values.max)
  val timeoutMapChisel: Map[SpeedMode.Type, UInt] = timeoutMapScala.map {
    case (mode, big) =>
  ...
  timeoutCyclesMax := MuxLookup(
    io.mainbandCtrlIo.freqSel,
    (timeoutMapScala.values.min - 1).U
  )(timeoutMapChisel.toSeq)
  ...
  when(
    timeoutCounter === (timeoutCyclesMax >> 2) && (currentState === LTState.sRESET)
  ) {
    resetMinWait := true.B
  }.elsewhen(
  ...
  switch(currentState) {
    is(LTState.sRESET) {
      ...
      when(
        pwrGood && sbPllLock && mbPllLock && resetMinWait && triggerTraining
      ) {
